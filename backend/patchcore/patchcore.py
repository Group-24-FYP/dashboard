...
class PatchCore(torch.nn.Module):
    ...
    def _fill_memory_bank(self, input_data):
       ...
       # print(features[0].shape)
        ...
        print("fiting anomaly score")
        self.anomaly_scorer.fit(detection_features=[features])

    def predict(self, image):
        if len(image) == 1:
            print("len", 1)
            return self._predict(image[0].unsqueeze(axis=0))
        else:
            return self._predict_dataloader(image)

    def _predict_dataloader(self, all_images):
        ...
        for i in range(len(all_images)):
            #print(img)
            if i == 0:
                _scores, _masks = self._predict(all_images[i][0].unsqueeze(axis=0), i)
                scores.append(_scores)
                masks.append(_masks)
            else:
                _scores, _ = self._predict(all_images[i][0].unsqueeze(axis=0), i)
                scores.append(_scores)
            
                #print('all', all_score)
                #print('mask', all_mask)
                #_scores, _masks = self._predict(image)
                #for score, mask in zip(all_score, all_mask):
                #    print(score)
                #    scores.append(score)
                #    masks.append(mask)
    ...
    def _predict(self, images, i=0):
        """Infer score and mask for a batch of images."""
        #images = np.numpy(images)
        #print(images)
        ...
        with torch.no_grad():
           ...
           # print('feature shape', features.shape)
            ...
            if i == 0:
                ...
                #print('mask shape',np.array(masks).shape, masks)

            ######image_scores = softmax_value  ########################
        #print(image_scores)
        #print(np.array(masks).shape)
        return [score for score in image_scores], masks if i == 0 else [score for score in image_scores]
